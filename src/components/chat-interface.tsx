...
const SectionHead = ({
  ...
  <h4 className={`font-semibold ${color} text-base sm:text-lg flex items-center gap-2`}>
    {icon}
    {title}
  </h4>
)
...
const Th = ({
  ...
  <th
    className={`bg-gray-900 text-gray-300 px-3 py-2 text-left font-semibold ${className}`}
  >
    {children}
  </th>
)
...
const Td = ({
  ...
  <td
    className={`px-3 py-2 border-t border-gray-700 align-top text-gray-200 ${className}`}
  >
    {children}
  </td>
)
...
const MobileCard = ({
  ...
  <div className="bg-gray-900/40 border border-gray-700 rounded-lg p-3">
    {title && <p className="text-gray-100 font-medium mb-2">{title}</p>}
    <div className="space-y-1">
      {rows
        .filter((r) => r.value !== undefined && r.value !== "")
        .map((r, i) => (
          <div key={i} className="flex items-start gap-2">
            <span className="text-gray-400 min-w-[96px] text-xs">{r.label}</span>
            <span className="text-gray-200 text-xs">{r.value as React.ReactNode}</span>
          </div>
        ))}
    </div>
  </div>
...
export default function ChatInterface() {
      ...
      if (storedData) {
        ...
        if (parsed.messagesBySession) {
          Object.keys(parsed.messagesBySession).forEach((sessionId) => {
            parsed.messagesBySession[sessionId] = parsed.messagesBySession[sessionId].map(
              (msg: Message) => ({
                ...msg,
  ...
  const saveSessionData = useCallback(
    (sessionData: Partial<StoredSessionData>) => {
      try {
        const currentData =
    ...
    try {
      ...
      if (sessionToDelete.id === currentSessionId) {
        if (updatedSessions.length > 0) {
          ...
          setMessages(updatedMessagesBySession[firstSession.id] || [])
          const finalSessions = updatedSessions.map((s, index) => ({ ...s, active: index === 0 }))
  ...
  const renderInterventions = (interventions?: TreatmentInterventions | string[]) => {
    ...
    if (Array.isArray(interventions)) {
      return (
        <ul className="list-disc pl-5 space-y-1 text-xs sm:text-sm text-gray-300">
    ...
    return (
      <>
        ...
        <div className="grid gap-2 sm:hidden">
          {Object.entries(interventions).map(([k, v]) => (
            <MobileCard key={k} rows={[{ label: k.replace(/_/g, " "), value: String(v) }]} />
          ))}
        </div>
      </>
    )
  ...
  const renderDifferentialTier = (label: string, rows?: DiagnosisItem[], color = "text-gray-200") => {
    if (!rows || rows.length === 0) return null
    return (
      <div className="space-y-2">
        <h5 className={`text-sm sm:text-base font-semibold ${color}`}>{label}</h5>
        ...
        <div className="grid gap-2 sm:hidden">
          {rows.map((r, idx) => (
            <MobileCard
              ...
              title={r.diagnosis}
              rows={[
                { label: "Pre-test", value: r.pre_test_probability },
                { label: "Feature", value: r.discriminator || r.features || r.classic || r.consider_if || "-" },
                { label: "Rule-out / Confirm", value: r.rule_out || r.confirm || r.test || "-" },
                { label: "Post-test", value: r.post_test_probability || "-" },
              ]}
            />
          ))}
        </div>
  ...
  const renderAssistantMessage = (message: Message) => {
    // Try to parse JSON (strip code fences first)
          ...
          <CardContent className="pt-0">
            <Section>
              ...
              <div className="bg-blue-900/20 p-4 rounded-lg border-l-4 border-blue-500">
                <p className="text-gray-300 whitespace-pre-wrap break-words leading-relaxed text-sm sm:text-base">
                  {message.content}
                </p>
              </div>
            </Section>
    ...
    return (
      <Card className="bg-gray-800 border-gray-600 w-full max-w-full overflow-hidden">
        ...
        <CardContent className="space-y-6 pt-0">
          ...
          {parsedData.immediate_assessment && (
            <Section>
              ...
              <div className="bg-green-900/20 p-4 rounded-lg border-l-4 border-green-500">
                <p className="text-sm sm:text-base text-gray-300 mb-3 font-medium">
                ...
                {parsedData.immediate_assessment.complete_diagnosis && (
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="bg-green-800/20 p-3 rounded">
                      <p className="text-green-300 text-xs sm:text-sm font-semibold">Primary</p>
                      <p className="text-gray-200 text-sm">{parsedData.immediate_assessment.complete_diagnosis.primary}</p>
                    </div>
                    {parsedData.immediate_assessment.complete_diagnosis.secondary && (
                      <div className="bg-green-800/20 p-3 rounded">
                        <p className="text-green-300 text-xs sm:text-sm font-semibold">Secondary</p>
                        <p className="text-gray-200 text-sm">
                          {parsedData.immediate_assessment.complete_diagnosis.secondary.join(", ")}
                        </p>
                      </div>
                    )}
                    {parsedData.immediate_assessment.complete_diagnosis.complications && (
                      <div className="bg-green-800/20 p-3 rounded">
                        <p className="text-green-300 text-xs sm:text-sm font-semibold">Complications</p>
                        <p className="text-gray-200 text-sm">
                          {parsedData.immediate_assessment.complete_diagnosis.complications.join(", ")}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <MobileCard
                    ...
                    title={
                      <span className="inline-flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-300">{a.condition}</span>
                      </span> as unknown as string
                    }
                    rows={[
                      { label: "Action", value: a.action },
                      { label: "Red flag", value: a.red_flag },
                      { label: "Evidence", value: a.evidence },
                    ]}
                  />
                ))}
              </div>
                ...
                <Section>
                  ...
                  <div className="bg-yellow-900/20 p-4 rounded-lg border-l-4 border-yellow-500">
                    <ul className="list-disc pl-5 space-y-1 text-sm text-gray-200">
                      {parsedData.urgent_protocol.tasks!.map((t, i) => (
                        <li key={i}>{t}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="grid gap-2 sm:hidden">
                    {parsedData.diagnostics.map((d, i) => (
                      <MobileCard
                        ...
                        title={d.test}
                        rows={[
                          { label: "Sensitivity", value: d.sensitivity },
                          { label: "Specificity", value: d.specificity },
                          { label: "Use when", value: d.use_when },
                        ]}
                      />
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                ...
                {parsedData.differential_diagnosis.checklist?.length ? (
                  <div className="bg-gray-900/40 p-3 rounded border border-gray-700">
                    <p className="text-xs sm:text-sm font-semibold text-gray-300 mb-2">Checklist</p>
                    <ul className="list-disc pl-5 text-xs sm:text-sm text-gray-300 space-y-1">
                      {parsedData.differential_diagnosis.checklist.map((c, i) => (
                        <li key={i}>{c}</li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="bg-green-900/20 p-4 rounded-lg border-l-4 border-green-500 mb-3">
                  <p className="text-sm sm:text-base text-gray-200 mb-1">
                    <span className="font-semibold text-green-300">
                      {parsedData.working_diagnosis.primary}
                    </span>
              ...
              {parsedData.primary_diagnosis_details && (
                <div className="grid sm:grid-cols-3 gap-3">
                  {parsedData.primary_diagnosis_details.pathophysiology && (
                    <div className="bg-gray-900/40 p-3 rounded border border-gray-700">
                      <p className="text-[11px] sm:text-xs font-semibold text-gray-300 mb-1">Pathophysiology</p>
                  ...
                  {parsedData.primary_diagnosis_details.diagnostic_criteria && (
                    <div className="bg-gray-900/40 p-3 rounded border border-gray-700">
                      <p className="text-[11px] sm:text-xs font-semibold text-gray-300 mb-1">Diagnostic Criteria</p>
                      <p className="text-sm text-gray-200">
                        {parsedData.primary_diagnosis_details.diagnostic_criteria}
                      </p>
                    </div>
                  ...
                  {parsedData.primary_diagnosis_details.natural_history && (
                    <div className="bg-gray-900/40 p-3 rounded border border-gray-700">
                      <p className="text-[11px] sm:text-xs font-semibold text-gray-300 mb-1">Natural History</p>
                      <p className="text-sm text-gray-200">
                        {parsedData.primary_diagnosis_details.natural_history}
                      </p>
                    </div>
              ...
              <div className="space-y-3">
                {parsedData.treatment_by_problem.map((t, i) => (
                  <div key={i} className="bg-blue-900/20 p-4 rounded-lg border-l-4 border-blue-500">
                    <p className="text-sm sm:text-base font-semibold text-blue-300 mb-3">{t.problem}</p>

                    {t.definitive_treatment && (
                      <div className="mb-3">
                        <p className="text-[11px] sm:text-xs font-semibold text-blue-200 mb-2">Definitive Treatment</p>
                        {renderKeyValueMini({
                    ...
                    {t.interventions && (
                      <div className="mb-3">
                        <p className="text-[11px] sm:text-xs font-semibold text-blue-200 mb-2">Interventions</p>
                    ...
                    {t.symptom_control && (
                      <div className="mb-3">
                        <p className="text-[11px] sm:text-xs font-semibold text-blue-200 mb-2">Symptom Control</p>
                        {renderKeyValueMini({
                    ...
                    {t.monitoring?.length ? (
                      <div className="mb-2">
                        <p className="text-[11px] sm:text-xs font-semibold text-blue-200 mb-1">Monitoring</p>
                        <ul className="list-disc pl-5 text-xs sm:text-sm text-gray-300 space-y-1">
                          {t.monitoring.map((m, idx) => (
                            <li key={idx}>{m}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {t.goal && (
                      <p className="text-xs sm:text-sm text-gray-300">
                        <span className="font-semibold">Goal:</span> {t.goal}
                      </p>
                    )}
                    {t.duration_limit && (
                      <p className="text-xs sm:text-sm text-gray-300">
                        <span className="font-semibold">Duration limit:</span> {t.duration_limit}
                      </p>
                    )}
                    {t.evidence && (t.evidence.nnt || t.evidence.reference) && (
                      <p className="text-[11px] text-gray-400 mt-2">
                        Evidence: {t.evidence.nnt ? `NNT ${t.evidence.nnt}` : ""}{" "}
                        {t.evidence.reference ? `• ${t.evidence.reference}` : ""}
                      </p>
                    )}
                  </div>
                ))}
          ...
          {/* Vestibular Rehab */}
          {parsedData.vestibular_rehab &&
            (parsedData.vestibular_rehab.start_time || parsedData.vestibular_rehab.exercises?.length) && (
              <Section>
                ...
                {parsedData.vestibular_rehab.start_time && (
                  <p className="text-sm text-gray-200 mb-2">
                    <span className="font-semibold">Start:</span> {parsedData.vestibular_rehab.start_time}
                  </p>
                )}
                ...
                <div className="grid gap-2 sm:hidden">
                  {parsedData.vestibular_rehab.exercises?.map((ex, i) => (
                    <MobileCard
                      ...
                      title={ex.type}
                      rows={[
                        { label: "Technique", value: ex.technique },
              ...
              <div className="bg-purple-900/20 p-4 rounded-lg border-l-4 border-purple-500">
                {parsedData.disposition.admit_if?.length ? (
                  <div className="mb-3">
                    ...
                    <ul className="text-sm text-gray-200 space-y-1 list-disc pl-5">
                      {parsedData.disposition.admit_if.map((d, i) => (
                        <li key={i}>{d}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {parsedData.disposition.discharge_if_all?.length ? (
                  <div>
                    ...
                    <ul className="text-sm text-gray-200 space-y-1 list-disc pl-5">
                      {parsedData.disposition.discharge_if_all.map((d, i) => (
                        <li key={i}>{d}</li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <div className="grid gap-2 sm:hidden">
                      {parsedData.discharge_instructions.return_immediately.map((it, i) => (
                        <MobileCard
                          key={i}
                          rows={[
                            { label: "Symptom", value: it.symptom },
                            { label: "Concern", value: it.concern },
                          ]}
                        />
                      ))}
                    </div>
                    ...
                    <div className="grid gap-2 sm:hidden">
                      {parsedData.discharge_instructions.follow_up.map((f, i) => (
                        <MobileCard
                          key={i}
                          rows={[
                            { label: "Provider / Test", value: f.provider || f.test },
                            { label: "When / Condition", value: f.timeframe || f.condition },
                          ]}
                        />
                      ))}
                    </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {parsedData.evidence_base?.length ? (
                <Section>
                  ...
                  <div className="grid gap-2 sm:hidden">
                    {parsedData.evidence_base.map((e, i) => (
                      <MobileCard key={i} title={e.topic} rows={[{ label: "Citation", value: e.citation }]} />
                    ))}
                  </div>
                </Section>
              ) : null}
              ...
              {parsedData.reassess_if?.length ? (
                <Section>
                  ...
                  <div className="grid gap-2 sm:hidden">
                    {parsedData.reassess_if.map((r, i) => (
                      <MobileCard key={i} rows={[{ label: "Trigger", value: r.trigger }, { label: "Action", value: r.action }]} />
                    ))}
                  </div>
                </Section>
              ) : null}
  ...
  if (isLoadingHistory) {
    return (
      <div className="flex h-dvh bg-[#212121] text-white items-center justify-center">
  ...
  return (
    <div className="flex h-dvh bg-[#212121] text-white relative">
      ...
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:relative z-50 lg:z-auto w-[82vw] max-w-80 bg-[#171717] border-r-2 border-gray-600 flex flex-col h-full transition-transform duration-300 ease-in-out shadow-xl lg:shadow-none`}
      >
        ...
        <ScrollArea className="flex-1 p-2">
          {sessions.map((session) => (
            ...
            >
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <DropdownMenu>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-gray-700 p-3 rounded-lg border-l-4 border-blue-500">
                    ...
                    {sessionToDelete.lastMessage && (
                      <p className="text-xs text-gray-400 mt-1 truncate">&quot;{sessionToDelete.lastMessage}&quot;</p>
                    )}
                  </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header */}
        <header className="flex justify-between items-center p-3 sm:p-4 border-b border-gray-700 bg-[#212121]/80 backdrop-blur-sm">
          <Button
            variant="ghost"
            size="icon"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg sm:text-xl font-semibold">ECDS v5.0</span>
            <ChevronDown className="w-4 h-4 text-gray-400" />
          </div>

          <div className="text-[10px] sm:text-xs md:text-sm text-gray-400 hidden xs:block">
            Active: {currentSessionId.substring(0, 8)}… ({messages.length})
          </div>
        </header>
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full px-4">
                <div className="text-center mb-6 sm:mb-8">
                  <Stethoscope className="w-12 h-12 sm:w-16 sm:h-16 text-blue-400 mx-auto mb-4" />
                  <h1 className="text-2xl sm:text-4xl font-normal mb-4 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 max-w-2xl w-full">
                  {[
                    ...
                    <button
                      key={index}
                      className="text-left p-3 sm:p-4 border border-gray-600 rounded-xl hover:bg-gray-700/50 transition-all duration-200 hover:border-gray-500 group"
                      onClick={() => setInput(item.desc)}
                    >
                      <div className="font-medium mb-2 group-hover:text-blue-400 transition-colors text-sm sm:text-base">
                        {item.title}
                      </div>
              ...
              <div className="max-w-6xl mx-auto space-y-3 sm:space-y-5 min-h-full">
                {messages.map((message) => (
                  <div key={message.id} className="group">
                    <div className={`flex gap-2 sm:gap-4 ${message.role === "user" ? "justify-end" : ""}`}>
                      ...
                      <div
                        className={`w-full ${
                          message.role === "user"
                            ? "bg-blue-600 text-white rounded-2xl px-3 py-2 sm:px-4 sm:py-3 max-w-[92%] xs:max-w-[88%] md:max-w-[80%] ml-auto"
                            : "max-w-full"
                        }`}
                      >
                        {message.role === "assistant" ? (
                          ...
                          <div className="whitespace-pre-wrap leading-relaxed break-words text-white text-sm sm:text-base">
                            {message.content}
                          </div>
                        )}
                      </div>
        ...
        {/* Input Area */}
        <div className="p-2 sm:p-4 border-t border-gray-700 bg-[#212121]/80 backdrop-blur-sm">
          <div className="max-w-6xl mx-auto">
            <form onSubmit={handleSubmit} className="relative">
              <Input
                ...
                className="w-full bg-[#2f2f2f] border-gray-600 rounded-2xl sm:rounded-3xl py-3 sm:py-4 px-4 sm:px-6 pr-12 sm:pr-16 text-white placeholder-gray-400 text-sm sm:text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                disabled={isLoading}
                maxLength={2000}
              />
              ...
              <div className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                <Button
                  type="submit"
                  size="icon"
                  className="rounded-full w-8 h-8 sm:w-9 sm:h-9 bg-blue-600 text-white hover:bg-blue-700 transition-all duration-200 disabled:opacity-50"
            ...
            <div className="text-center text-gray-400 text-[11px] sm:text-sm mt-2 sm:mt-3">
              Emergency Clinical Decision Support • {input.length}/2000
            </div>
          </div>
        </div>
      </main>
